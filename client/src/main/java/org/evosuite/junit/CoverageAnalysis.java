...
public class CoverageAnalysis {
	...
	private static void analyzeCoverageCriterion(List<JUnitResult> results, Properties.Criterion criterion) {
		...
		if(criterion == Criterion.EPAEXCEPTION || criterion == Criterion.EPAADJACENTEDGES || criterion == Criterion.EPATRANSITIONMINING
			...
			//To save inferred epa
			//It need measureCoverage for a criterion that not required epaxml
			//I use this hack to save inferred epa for a test suite generated by randoop
			if(Properties.FORCE_INFERRED_EPA && !EPAUtils.currCriteriaRequireEPAXML())
				EPAUtils.saveInferredEPA(executionResults, Properties.INFERRED_EPA_XML_PATH);
			...
			switch(criterion)
			{
				...
				case EPATRANSITIONMINING:
					...
					totalGoals += EPATransitionMiningCoverageFactory.getGoals().size();
					break;
				case EPAEXCEPTIONMINING:
					...
					totalGoals += EPAExceptionMiningCoverageFactory.getGoals().size();
					break;
				case EPAADJACENTEDGESMINING:
					...
					totalGoals += EPAAdjacentEdgesMiningCoverageFactory.getGoals().size();
					break;
				default:
					break;
			}
